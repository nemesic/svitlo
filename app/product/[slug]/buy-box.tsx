...
export default function BuyBox({
  ...
  return (
    // sticky стоит и здесь, и на галерее — это безопасно и делается
    // намеренно.
    // Запас хода у залипания = высота грид-области минус высота самого
    // элемента. Грид-область у обеих колонок одна и та же — высота строки,
    // то есть высота более длинной из них. Значит у длинной колонки запас
    // ровно нулевой: она не залипает и ведёт себя обычно, ничего не
    // обрезается. Короткая забирает весь запас и держится.
    // Какая из них короче — зависит от товара: число кадров, длина
    // описания, количество цветов и размеров у всех разные. Поэтому
    // выбор отдан браузеру, а не зашит в разметку.
    // self-start обязателен: без него грид-элемент растянулся бы на всю
    // высоту строки, запас хода стал бы нулевым всегда и sticky молча
    // ничего не делал бы.
    <div className="pl-0 pt-3.5 md:sticky md:top-[90px] md:self-start md:pl-[clamp(0px,4vw,56px)]">
